...
function displayQuestion() {
    ...
    if (currentQuestionIndex < quizData.length) {
        ...
        document.getElementById('choice-E').textContent = `E. ${currentQuestion.Choice_E}`;
...
// 解答の判定
function checkAnswer(selectedButton, correctKey) {
    ...
    // 正解ボタン取得
    const correctButton = Array.from(allButtons)
        .find(btn => btn.dataset.answerKey === correctKey);

    const correctText = correctButton
        ? correctButton.textContent
        : '';

    if (selectedKey === correctKey) {
        score++;
        feedbackElem.textContent = '正解☆彡';
        feedbackElem.style.color = '#4caf50';
        selectedButton.classList.add('correct');
    } else {
        feedbackElem.textContent = '残念！';
    ...
    // ★ 正解を必ず文章で表示
    correctAnswerElem.textContent = `正解は「${correctText}」です。`;
...
nextButton.style.display = 'block';    
    ...
    explanationElem.innerHTML = `
        <strong>正解：</strong>${correctText}<br>
        <strong>解説：</strong>${questionSet[currentQuestionIndex].Explanation}
    `;
...
restartButton.onclick = () => {
    ...
    choicesArea.innerHTML = `
        ...
        <button class="choice-btn" id="choice-E" data-choice="E"></button>`;
